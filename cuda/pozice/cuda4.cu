...
__global__ void kernel_vlneni( CUDA_Pic colorPic, CUDA_Pic bwPic, int posun, int index )
{
    ...
    int  positionx;
        positiony = y + 128;
        positionx = x + 128;      
    float position = positionx * bwPic.Size.y + positiony + index * posun;
     bwPic.PData[(int)position] = bgr;
    
    /*Resize*/
    //int position = (x/4)*bwPic.Size.y +y;
    /*Rotace*/
    //int position= x+bwPic.Size.x*y;
    
    //doprava
    //bwPic.PData[x * bwPic.Size.y + (bwPic.Size.y - y)] = colorPic.PData[y * sizex + x]; 
   // float position = x * bwPic.Size.y + (bwPic.Size.y - y);
   // bwPic.PData[(colorPic.Size.y*(colorPic.Size.x-x))+y] = colorPic.PData[ y * colorPic.Size.x +x];
   //rotate[(sizex - x) * sizey + y] = original[y * sizex + x];
